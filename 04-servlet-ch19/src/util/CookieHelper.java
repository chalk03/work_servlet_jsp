...
public class CookieHelper {
	...
	public CookieHelper(HttpServletRequest req) {
		...
		if (cookies != null) {
			for (int i = 0; i < cookies.length; i++) {
				// cookies 배열에 있는 key와 값들을 cookieMap에 put메소드를 이용하여 저장
	...
	/*
	 * key, value, path, maxAge를 전달받아 Cookie 객체를 생성하는 메소드 path: 해당 하위 경로까지 쿠키를 사용할 수
	 * 있다. 예) cookie.setPath("/ch09"); -> /ch09 포함해서 이하경로까지 해당 쿠키를 사용할 수 있다. maxAge:
	 * 해당 시간동안 cookie가 살아있게 설정하는 것 (초 단위) -1: 무제한, 0: 삭제
	 */
	...
	/*
	 * domain 예) cookie.setDomain(".koitt.com"); -> www.koitt.com, main.koitt.com
	 * 등등의 도메인에서만 해당 쿠키를 사용할 수 있게 설정 cookie.setDomain("www.koitt.com"); ->
	 * www.koitt.com에서만 해당 쿠키를 사용할 수 있다.
	 */
